...
- `.addTarget` 增加一个卡牌使用目标，可行目标由[实体查询语法（旧版）](../query-legacy.md)给出。
...
在 builder chain 中指定 `.toStatus` 后，即表明该牌打出后将产生角色状态。该链方法的参数为角色状态的附属目标的[实体查询（旧版）](../query-legacy.md)字符串，随后的描述均为该[角色状态](./entity.md)的定义。例：
