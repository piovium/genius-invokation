...
有两种方式描述操作：
1. **声明域描述**，直接在 GTS 定义块中指定操作方法
2. **脚本域描述**，在块内以 `:` 开头执行任意 JavaScript 语句
...
在 GTS 中，写在 `on` 块或 `define skill` 块顶层、以 `:` 开头的是脚本域代码。脚本域代码中的 `:` 前缀表示访问当前技能执行上下文（`SkillContext`）。对于简单的操作（如 `:increaseDamage(3)`）可直接在声明域中以 `:` 开头调用；对于需要分支循环等复杂逻辑，直接在脚本域中写任意 JavaScript 语句即可：
...
`:` 后的表达式在 `SkillContext` 对象上执行，调用 `c` 上的一些方法就可以完成对游戏状态的读取和更改。当技能被使用时，核心库会生成好 `c` 实参，然后执行脚本域代码。
