...
- `extension(idHint, type)(initState)`：开始一个扩展点的定义。`idHint` 为扩展点的 id 提示，通常与使用该扩展点的实体/技能/手牌相关联；全局的所有扩展点的 `idHint` 不能重复。`type` 为 ArkType 语法的类型说明，用于编辑器提示。`initState` 为该扩展点所扩展的状态变量的结构。与实体不同，此结构可使用任意的对象、数组甚至 `Set` 与 `Map`。
