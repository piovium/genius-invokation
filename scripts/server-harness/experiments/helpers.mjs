...
export async function startExperimentServer() {
  ...
  if (!runtime.available)
    throw new Error(
      `Real WebSocket experiments require Node: ${runtime.reason}. Use Node 24+ and install the isolated experiment dependency.`,
    );
  const child = spawn(
    runtime.command,
    [fileURLToPath(new URL("./server.mjs", import.meta.url)), "--selftest"],
  ...
  try {
    ready = await new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => finish(new Error(`Node fixture startup timed out: ${stderr}`)),
        10000,
      );
      const onExit = (code) =>
        finish(new Error(`Node fixture exited (${code}): ${stderr}`));
      ...
      const onData = (chunk) => {
        ...
        if (stdout.length > 4096)
          return finish(new Error("Unexpected fixture startup output"));
        if (!stdout.includes("\n")) return;
        try {
          const value = JSON.parse(stdout.split("\n")[0]);
          if (
            value.type !== "harness-ready" ||
  ...
  const request = async (path, options) => {
    ...
    if (!response.ok)
      throw new Error(
        `Fixture HTTP ${response.status}: ${await response.text()}`,
      );
  ...
  return {
    origin,
    baseUrl: `${origin}/api`,
    runtime: { name: "Node", version: runtime.version, pid: child.pid },
    stop,
    createRoom: (options = {}) =>
      request("/__harness/rooms", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(options),
      }),
    stats: (roomId) =>
      request(`/__harness/rooms/${encodeURIComponent(roomId)}/stats`),
  };
}
...
export async function connectSocket(url, { timeoutMs = 3000 } = {}) {
  ...
  function drain() {
    for (const waiter of pending) {
      const index = messages.findIndex(
        (entry) => !entry.consumed && waiter.predicate(entry.value, entry),
      );
      if (index !== -1) {
        ...
        waiter.reject(
          failure ||
            new Error(`Socket closed ${closed.code}: ${closed.reason}`),
        );
      }
    }
  }
  socket.addEventListener("message", (event) => {
    try {
      ...
      messages.push({
        value: binary
          ? decodeGameFrame(new Uint8Array(event.data))
  ...
  return {
    ...
    sendResponse: (id, response = Uint8Array.of(0x12, 0)) =>
      socket.send(encodeGameFrame({ type: "actionResponse", id, response })),
