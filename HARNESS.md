...
| E1／engine、gts-engine、desktop | data、其他 check、GTS LSP、TS/TSX tsserver 均实际使用 TNB | 各路径的 resolved module、精确版本、native 运行与产物身份；横幅仅作辅助 |
...
| L1／desktop | 在 GTS checkout 里从仓库根与 `examples` 工作区冷启动，开发路径（`--extensionDevelopmentPath`）和打包安装的 VSIX 两种模式；真实 GamingTS 扩展宿主 | GTS LSP 与 TS/TSX tsserver 均可用，无模块缺失、原生加载错误或重启循环；Node 测试不能替代 Electron，开发路径也不能替代已安装 VSIX |
| L1／gts-lsp | 不经编辑器，直接以 stdio 驱动 `@gi-tcg/gts-language-server`，使用真实 GamingTS provider 集 | 诊断、hover、definition、completion、signature help 与最新文档版本关联；resolved TNB 版本、native addon 与 RPC 计数可核验；编辑器证据不能替代语言服务自身的原生身份 |
...
核心 harness 自测与独立复核完成后，协调者审查并切换 phase、更新 seal、再次 `verify`／`selftest`，再通过 `task ROLE` 生成任务。角色为 integration、gts、tnb，写入边界以 contract 为准。分派时原样传递生成任务，并追加具体实现目标；不复用此前被中断的旧分派。新发现需要越过角色边界时，协调者重新分配，不能擅自修改其他 worktree。
...
- 根依赖替换对声明构建、Vitest 和各 SDK 解析路径的影响未知。
