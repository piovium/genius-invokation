...
export function CurrentTurnHint(props: CurrentTurnHintProps) {
  ...
  return (
    <Show when={props.phase <= PbPhaseType.ROLL}>
      <div
        class="h-8 w-24 flex items-center justify-center rounded-full b-2 current-turn-hint text-color-[var(--fg-color)] border-[var(--fg-color)] bg-[var(--bg-color)]"
        data-opp={props.opp}
      >
        <AutoResizeText minFontSize={10} class="text-center font-bold w-20">
          {t(
