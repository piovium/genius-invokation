...
export interface BuildStateOptions {
  /** 使用指定的卡牌数据；默认 `getData(stateProp.dataVersion)` */
  data?: GameData;
  /** 实体 id 生成器；默认使用模块级全局计数器 */
  nextId?: () => number;
}

/** 将 JSX 状态描述构造为 `GameState`（不启动对局） */
export function buildState(
  ...
  const data = options.data ?? getData(stateProp.dataVersion);
