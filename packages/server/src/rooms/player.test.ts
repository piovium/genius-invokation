...
const config: RoomConfig = {
  initTotalActionTime: 45, rerollTime: 40, roundTotalActionTime: 60, actionTime: 25,
  watchable: false, private: true, allowGuest: true, gameVersion: CURRENT_VERSION,
};
...
const player = (id = "guest-test", sessionId = "session-test") => {
  const instance = new Player({ id, isGuest: true, name: id, deck: { characters: [], cards: [] } }, sessionId);
  instance.setTimeoutConfig(config);
  return instance;
};
const hasCode = (code: string) => (error: unknown) => error instanceof RoomCommandError && error.code === code;

test("real Player accepts once synchronously across concurrent callers and replays its original ACK", async () => {
  const instance = player();
  let accepted = 0;
  const pending = instance.rpc(request).then((value) => { accepted++; return value; });
  try {
    assert.throws(() => instance.receiveResponse(1, response), hasCode("FUTURE_RPC"));
    assert.throws(() => instance.receiveResponse(0, Uint8Array.of(0)), hasCode("INVALID_RESPONSE"));
    ...
    assert.throws(() => instance.receiveResponse(0, Uint8Array.of(0x12, 2, 8, 2)), hasCode("CONFLICT"));
    assert.deepEqual(await pending, PbRpcResponse.decode(response));
    assert.equal(accepted, 1);
    assert.equal(instance.receiveResponse(0, response), first, "Reconnect after engine IO resumes still obtains the original ACK");
...
test("the last 32 accepted RPCs remain recoverable while evicted commands never execute again", async () => {
  ...
  try {
    for (let id = 0; id < 48; id++) {
      const pending = instance.rpc(request).then(() => { executions++; });
      instance.receiveResponse(id, response);
      await pending;
    }
    assert.throws(() => instance.receiveResponse(15, response), hasCode("STALE_RPC"));
...
test("finished subscriptions expose the sync boundary and permit recovery of a lost final ACK", async () => {
  ...
  let closed = false;
  instance.subscribe({ send: (event) => events.push(event), close: () => { closed = true; } });
  assert.ok(events.some((event) => event.type === "rpc" && event.data === null));
...
test("real engine switch-hands IO advances once and rejects invalid card choices before acceptance", async () => {
  ...
  const game = new Game(Game.createInitialState({
    decks: [{ characters: [], cards: [] }, { characters: [], cards: [] }],
    data: { characters: new Map(), entities: new Map(), attachments: new Map(), extensions: new Map() },
    randomSeed: 20260910,
  }));
  const pending = [Promise.withResolvers<number>(), Promise.withResolvers<number>()] as const;
  for (const who of [0, 1] as const) {
    game.players[who].io = players[who];
    players[who].onInitialized(who, game, players[who === 0 ? 1 : 0]);
    players[who].subscribe({
      send: (event) => { if (event.type === "rpc" && event.data) pending[who].resolve(event.data.id); },
      close: () => {},
    });
  }
  const finished = game.start();
  try {
    const [id0, id1] = await Promise.all([pending[0].promise, pending[1].promise]);
    assert.equal(game.state.phase, "initHands");
    assert.throws(() => players[0].receiveResponse(id0, Uint8Array.of(0x12, 2, 8, 2)), hasCode("INVALID_RESPONSE"));
    assert.equal(game.state.phase, "initHands");
    ...
    players[0].dispose(); players[1].dispose();
