...
function parseDeck(value: unknown): Deck {
  ...
  return {
    characters: list(record.characters, 3, "characters"),
    cards: list(record.cards, 30, "cards"),
  };
}
...
function roomFields(input: Record<string, unknown>): CreateRoomDto {
  ...
  const limits = {
    gameVersion: [0, VERSIONS.length - 1],
    initTotalActionTime: [0, 300],
    rerollTime: [25, 300],
    roundTotalActionTime: [0, 300],
    actionTime: [25, 300],
    randomSeed: [0, 2147483546],
  } as const;
