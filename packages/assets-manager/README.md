...
This package will fetch assets from the Static Data v5 API and manage them using Web API<sup>*</sup>.

> \* `fetch` and `Blob` API must be presented in the running environment.
> `AssetsManagerOption` is designed to be JSON-serializable so that can be safely passed through different runtime environment.
