...
function cloneCharacterState(character: CharacterState): CharacterState {
  return {
    [StateSymbol]: "character",
    id: character.id,
    definition: character.definition,
    variables: { ...character.variables },
    entities: character.entities.map(cloneEntityState),
  };
}

function cloneAttachmentState(attachment: AttachmentState): AttachmentState {
  return {
    [StateSymbol]: "attachment",
    id: attachment.id,
...
function cloneEntityState(entity: EntityState): EntityState {
  return {
    [StateSymbol]: "entity",
    id: entity.id,
    definition: entity.definition,
    variables: { ...entity.variables },
    attachments: entity.attachments.map(cloneAttachmentState),
  };
}
...
function clonePlayerState(player: PlayerState): PlayerState {
  return {
    [StateSymbol]: "player",
    who: player.who,
    initialPile: [...player.initialPile],
    pile: player.pile.map(cloneEntityState),
    activeCharacterId: player.activeCharacterId,
    hands: player.hands.map(cloneEntityState),
    characters: player.characters.map(cloneCharacterState),
    combatStatuses: player.combatStatuses.map(cloneEntityState),
    supports: player.supports.map(cloneEntityState),
    summons: player.summons.map(cloneEntityState),
    dice: [...player.dice],
    declaredEnd: player.declaredEnd,
    hasDefeated: player.hasDefeated,
    canCharged: player.canCharged,
    canPlunging: player.canPlunging,
    legendUsed: player.legendUsed,
    skipNextTurn: player.skipNextTurn,
    defeatedSwitching: player.defeatedSwitching,
    roundSkillLog: new Map(
      Array.from(player.roundSkillLog.entries(), ([key, value]) => [
        key,
        [...value],
      ]),
    ),
    phaseDamageLog: [...player.phaseDamageLog],
    phaseReactionLog: [...player.phaseReactionLog],
    removedEntities: [...player.removedEntities],
  };
}
...
export function cloneGameState(gameState: GameState): GameState {
  return {
    ...
    versionBehavior: { ...gameState.versionBehavior },
    iterators: { ...gameState.iterators },
    phase: gameState.phase,
    roundNumber: gameState.roundNumber,
    currentTurn: gameState.currentTurn,
    winner: null,
    players: [
      clonePlayerState(gameState.players[0]),
      clonePlayerState(gameState.players[1]),
    ],
    extensions: gameState.extensions.map((extension) => ({
      [StateSymbol]: "extension",
...
export function materializeGameState(gameState: GameState): GameState {
