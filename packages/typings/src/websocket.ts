...
export const MAX_GAME_FRAME_BYTES = MAX_GAME_PAYLOAD_BYTES + 24;
...
function checkId(id: number): void {
  if (!Number.isInteger(id) || id < 0 || id > 0xffff_ffff) invalid("id must be uint32");
}

function checkPayload(payload: Uint8Array): void {
  if (!(payload instanceof Uint8Array) || payload.byteLength === 0) invalid("missing protobuf payload");
  if (payload.byteLength > MAX_GAME_PAYLOAD_BYTES) invalid("protobuf payload exceeds 8 MiB");
}
...
function checkTimer(timer: GameRpcTimer): void {
  ...
  if (!timer || !Number.isFinite(timer.current) || !Number.isFinite(timer.total)) invalid("timer must contain finite numbers");
}
...
export function encodeGameFrame(frame: GameWireFrame): Uint8Array<ArrayBuffer> {
  const kind = frame?.type === "notification" ? 1 : frame?.type === "rpc" ? 2 : frame?.type === "actionResponse" ? 3 : invalid("unsupported kind");
  const id = frame.type === "notification" ? 0 : frame.type === "rpc" ? frame.data.id : frame.id;
  const payload = frame.type === "notification" ? frame.data : frame.type === "rpc" ? frame.data.request : frame.response;
  checkId(id);
  checkPayload(payload);
  const offset = kind === 2 ? 24 : HEADER_BYTES;
...
export function decodeGameFrame(input: ArrayBuffer | Uint8Array): GameWireFrame {
  const bytes = input instanceof ArrayBuffer ? new Uint8Array(input) : input;
  if (!(bytes instanceof Uint8Array)) invalid("expected ArrayBuffer or Uint8Array");
  ...
  const offset = kind === 2 ? 24 : HEADER_BYTES;
  if (bytes.byteLength < offset) invalid("truncated RPC timer");
