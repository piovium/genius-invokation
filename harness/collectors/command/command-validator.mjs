...
export function testSummaries(stdout) {
  ...
  const count = (rows, kind) => rows.reduce((sum,line)=>sum+Number(line.match(new RegExp('(\\d+) '+kind+'\\b'))?.[1]??0),0);
...
export async function packageInventory(root, repo, spec, gate, expectations) {
  ...
  for(const file of originalPaths.filter(file=>configPattern.test(file)||kind==='test'&&
    (expectations.typecheck?.[gate.repository]?.configFiles??[]).includes(file))) {
    if (selected && !selected.some(pkg=>file.startsWith(pkg.path+'/')) && file.includes('/')) continue;
    check(fs.existsSync(path.join(repo,file)) && fs.readFileSync(path.join(repo,file),'utf8').replaceAll('\r\n','\n').trimEnd()
  ...
  const currentPackages = currentPaths.filter(file=>file!=='package.json' && manifestPattern.test(file)).map(file=>{
    const manifest=JSON.parse(fs.readFileSync(path.join(repo,file)));return{path:normalize(path.dirname(file)),manifest};
  });
  if(kind==='build') {
    if(gate.repository==='gts') check(same(currentPackages.filter(p=>p.manifest.scripts?.build).map(p=>p.path).sort(),
      selected.map(p=>p.path).sort()),'GTS build selection changed');
    return selected.map(item=>{
      const pkg=currentPackages.find(p=>p.path===item.path);check(pkg?.manifest.scripts?.build,'Missing selected build package');
      return {...item,name:pkg.manifest.name,script:pkg.manifest.scripts.build};
    });
  }
  return currentPackages.filter(p=>p.manifest.scripts?.test).map(pkg=>{
    const originalFiles=originalPaths.filter(f=>f.startsWith(pkg.path+'/')&&testPattern.test(f));
    check(originalFiles.every(file=>currentPaths.includes(file)&&fs.existsSync(path.join(repo,file))),'An original test file was removed');
    ...
    const files=currentPaths.filter(f=>f.startsWith(pkg.path+'/')&&testPattern.test(f)).sort();
...
export function outputInventory(repo, packages, allowMissing=false) {
  ...
  function add(file) {
    ...
    if(!fs.existsSync(full)) { check(allowMissing,'Missing build artifact: '+file);results.push({file,missing:true});return; }
    const stat=fs.statSync(full);check(stat.isFile(),'Not an artifact file: '+file);
  ...
  function walk(dir) {
    ...
    for(const ent of fs.readdirSync(dir,{withFileTypes:true}).sort((a,b)=>a.name.localeCompare(b.name))) {
      if(ent.isSymbolicLink())throw new Error('Unexpected linked build output');
      if(ent.isDirectory())result.push(...walk(path.join(dir,ent.name)));
      else if(ent.isFile())result.push(path.join(dir,ent.name));
    }
    return result;
  }
  for(const pkg of packages) {
    for(const file of new Set([...(pkg.outputs??[]),...(pkg.declarations??[]).map(d=>d.file)]))add(pkg.path+'/'+file);
    for(const dir of pkg.directories??[]) {
      const files=walk(path.join(repo,pkg.path,dir));
      check(allowMissing||files.length>0,'Empty build output directory: '+pkg.path+'/'+dir);
...
export function validateTestProcess(events, pkg, repo) {
  ...
  check(events.some(e=>e.kind==='vitest'&&/[\\/]vitest[\\/]vitest\.mjs$/.test(e.module.path)),
    'Package did not load the actual Vitest CLI');
  const stdout=events.filter(e=>e.kind==='stdout').map(e=>Buffer.from(e.base64,'base64').toString('utf8')).join('');
  const stderr=events.filter(e=>e.kind==='stderr').map(e=>Buffer.from(e.base64,'base64').toString('utf8')).join('');
  ...
  check(report.summaries.length===1&&report.files.length===1&&report.executedTests>=pkg.minimumTests&&
    report.passedFiles>=pkg.files.length&&!report.failedTests&&!report.skippedTests&&!report.failedFiles&&!report.skippedFiles,
    'Missing, failed, skipped, or reduced tests in '+pkg.path);
  ...
  check(clean(stdout).split(/\r?\n/).some(line=>/\bRUN\s+v\S+\s+/.test(line)&&
    normalize(line).trim().endsWith(normalize(path.join(repo,pkg.path)))),'Vitest root differs from package');
...
export async function validate(evidence, { root, directory, contract, gate, expectations, nonce }) {
  try {
    ...
    check(evidence.runNonce===nonce&&evidence.gateId===gate.id&&evidence.platform===process.platform,'Wrong command provenance');
    ...
    const node=path.resolve(root,local.node),spec=contract.repositories[gate.repository],manager=path.resolve(root,local[spec.manager]);
    const repo=inside(root,spec.path),command=evidence.command;
    ...
    async function verifyCommand(cmd,args,label,timeout=gate.timeoutMs) {
      check(cmd?.executable===node&&stable(cmd.args)===stable(args)&&cmd.cwd===fs.realpathSync.native(repo)
        &&processVerdict(cmd)==='PASS'&&cmd.durationMs>0&&cmd.durationMs<=timeout&&Number.isFinite(Date.parse(cmd.startedAt)),
        'Fixed build/test command did not pass');
      const logs={};
    ...
    const childStart = Date.parse(childLifetime.startedAt), childEnd = Date.parse(childLifetime.finishedAt);
    ...
    const names=fs.readdirSync(observationDir).sort().map(f=>'command-observations/'+f);
    check(Array.isArray(evidence.observations)&&names.length>0&&
      same(evidence.observations.map(e=>e.file),names),'Missing, duplicate, or omitted process observations');
    const processes=[];
    const preloadHash=await hashFile(collector),nodeHash=await hashFile(node);
    async function verifyArtifact(item) {
      check(item&&item.path===fs.realpathSync.native(item.path)&&await hashFile(item.path)===item.sha256,'Executed artifact identity changed');
    }
    async function verifyNative(item,compilerFile) {
      ...
      check(manifest.name==='typescript-native-bridge'&&manifest.version===contract.tnbVersion,'Loaded compiler is not pinned TNB');
    ...
    for(const item of evidence.observations) {
      check(/^command-observations\/\d+-\d+-[a-f0-9-]{36}\.(?:jsonl|rpc\.log)$/.test(item.file),'Unexpected observation filename');
      const full=inside(directory,item.file);
      check(await hashFile(full)===item.sha256,'Process observation hash changed');
      if(!item.file.endsWith('.jsonl'))continue;
      const events=fs.readFileSync(full,'utf8').trim().split('\n').map(JSON.parse);
      const start=events[0];
      check(start?.kind==='start'&&events.filter(e=>e.kind==='start').length===1
      ...
      check(Number.isSafeInteger(start.pid)&&start.pid>0&&Number.isSafeInteger(start.threadId)&&start.threadId>=0&&
        item.file==='command-observations/'+start.pid+'-'+start.threadId+'-'+start.instanceId+'.jsonl'&&events.every((e,i)=>
        e.pid===start.pid&&e.threadId===start.threadId&&e.instanceId===start.instanceId&&e.runNonce===nonce&&e.gateId===gate.id&&Number.isSafeInteger(e.at)&&
        e.at>=Date.parse(command.startedAt)&&e.at<=Date.parse(command.startedAt)+command.durationMs+100&&
        (!i||e.at>=events[i-1].at)), 'Wrong process observation provenance/timing');
      if(start.entry)await verifyArtifact(start.entry);
      for(const ev of events)if(['entry','vitest','compiler','native'].includes(ev.kind)) {
        await verifyArtifact(ev.module);
        if(ev.kind==='compiler') {
          check(/^[a-f0-9]{64}$/.test(ev.compiledSha256),'Missing actual compiled input hash');
          const manifest=readJson(path.join(path.dirname(ev.module.path),'../package.json'));
        ...
        if(ev.kind==='native') {
          await verifyArtifact(ev.parent);
          check(events.some(c=>c.kind==='compiler'&&c.module.path===ev.parent.path&&c.at<=ev.at),
            'Native addon not loaded by observed compiler');
          await verifyNative(ev.module,ev.parent.path);
        }
      }
    ...
    if(gate.id.endsWith('tests')) {
      ...
      check(!/Unhandled (?:Errors|Rejection|Source Error)|TypeCheckError:/.test(logs.stdout+logs.stderr),'Unhandled Vitest failure');
      for(const pkg of packages) {
        const candidates=processes.filter(events=>events[0].isMainThread&&events[0].cwd===path.join(repo,pkg.path)&&
          events.some(e=>e.kind==='vitest'));
      ...
      if(gate.id==='main-tests') {
        ...
        const typedFiles=packages.flatMap(p=>p.files).filter(f=>/\.(?:test|spec)-d\.[cm]?[jt]sx?$/.test(f));
        check(expectations.typecheck.main.typedFiles.every(f=>typedFiles.includes(f)),'Original typed tests omitted');
        const typedPackages=packages.filter(p=>p.files.some(f=>typedFiles.includes(f)));
        for(const pkg of typedPackages) {
          const candidates=processes.filter(events=>{
            const s=events[0],args=s.argv.slice(2),project=args.indexOf('-p');
            return s.isMainThread&&s.cwd===path.join(repo,pkg.path)&&s.entry&&
              /[\\/]typescript(?:-native-bridge)?[\\/]bin[\\/]tsc$/.test(s.entry.path)&&project>=0&&
      ...
      for(const pkg of packages)for(const tool of pkg.tools) {
        check(processes.some(events=>{
          const s=events[0];return s.isMainThread&&s.cwd===path.join(repo,pkg.path)&&s.packageName===pkg.name&&
            /^build(?::|$)/.test(s.lifecycle??'')&&s.entry&&normalize(s.entry.path).endsWith('/'+tool)&&
            events.some(e=>e.kind==='entry'&&e.module.path===s.entry.path)&&
            events.at(-1).kind==='exit'&&events.at(-1).code===0;
        }),'Selected build tool did not execute: '+pkg.path+' '+tool);
      }
      check(processes.some(events=>events.some(e=>e.kind==='native')),'Build did not load native TNB');
      const actual=outputInventory(repo,packages);
      check(actual.every(o=>o.size>0)&&stable(actual)===stable(evidence.afterOutputs),'Missing/empty/changed real build artifacts');
      ...
      check(fs.readFileSync(inside(directory,'public-consumer.ts'),'utf8')===consumerSource(packages,repo),'Public consumer was changed or reduced');
      check(stable(readJson(inside(directory,'public-consumer.tsconfig.json')))===stable(consumerConfig),'Public consumer configuration changed');
      ...
      check(result.diagnostics?.length===0&&result.rpcDelta>0&&result.native?.length>0,'Public declaration consumption failed or did not execute native checks');
      await verifyArtifact(result.source);await verifyArtifact(result.config);await verifyArtifact(result.compiler);await verifyArtifact(result.manifest);
      for(const native of result.native)await verifyNative(native,result.compiler.path);
      const require=createRequire(path.join(repo,'package.json'));
      check(result.compiler.path===fs.realpathSync.native(require.resolve('typescript'))&&result.manifest.name==='typescript-native-bridge'&&
        result.manifest.version===contract.tnbVersion&&result.source.path===fs.realpathSync.native(path.join(directory,'public-consumer.ts')),
