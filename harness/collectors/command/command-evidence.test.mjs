...
function packageEvents(name, log = nativeLog) {
  ...
  const prefix = `${pathName} test:`;
  const stdout = log.split(/\r?\n/).filter(line => line.startsWith(prefix)).map(line => line.slice(prefix.length)).join('\n')
  ...
  const pkg = { path: pathName, name: `test-${name}`, script: ['language-server', 'tsc'].includes(name) ? 'vitest run' : 'vitest',
    files: Array.from({ length: counts[name] }, (_, index) => `${pathName}/${index}.test.ts`), minimumTests: expected.testMinimums.gts[pathName] };
...
test('actual pnpm-prefixed GTS output contains all 89 tests and 15 files', () => {
  ...
  assert.equal(summary.executedTests, 89);
  assert.equal(summary.passedFiles, 15);
  assert.equal(summary.skippedTests, 0);
  assert.equal(summary.summaries.length, 6);
});
for (const name of ['local', 'runtime', 'transpiler', 'prettier-plugin', 'language-server', 'tsc']) test(`real per-package Vitest output validates: ${name}`, () => {
  const { pkg, events } = packageEvents(name);
  assert.ok(validateTestProcess(events, pkg, fakeRepo).report.executedTests > 0);
});
...
test('mentioning a package path alongside another package summary cannot substitute its tests', () => {
  const { pkg, events } = packageEvents('local');
  const other = packageEvents('runtime').events[2];
...
test('public consumer covers all reviewed declaration entries and checks named API/any/missing imports', () => {
  for (const repo of ['main', 'gts']) {
    const packages = expected.buildPackages[repo];
    const source = consumerSource(packages, fakeRepo);
    const declarations = packages.flatMap(pkg => pkg.declarations);
...
test('required empty/missing build outputs fail; unchanged timestamps are accepted', () => {
  ...
  try {
    ...
    const a = outputInventory(temp, packages), b = outputInventory(temp, packages);
...
test('native compiler/typechecker gates cannot accept a bare exit-zero envelope', async () => {
  ...
  for (const id of ['gts-build', 'gts-tests', 'main-build', 'main-tests']) {
    const result = await validate({ runNonce: 'negative', gateId: id, platform: process.platform, command: { exitCode: 0 } },
      { root, directory: root, contract, gate: contract.gates.find(gate => gate.id === id), expectations: expected, nonce: 'negative' });
...
test('frozen baseline inventories cover examples/local and all original main test files; deletions/script changes fail', async () => {
  ...
  try {
    ...
    for (const p of baseline) {
      fs.mkdirSync(path.join(temp, p.path), { recursive: true });
      ...
      for (const f of p.files) {
        ...
        fs.writeFileSync(path.join(temp, f), '// Inventory-only fixture for original path\n');
      }
    }
    git(['add', '.']);
...
test('passive preload preserves a real child stdout and observes its entry and lifetime', () => {
  ...
  try {
    ...
    fs.writeFileSync(child, 'process.stdout.write("unchanged output\\n");');
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.equal(result.stdout, 'unchanged output\n');
    const log = fs.readdirSync(temp).find(file => file.endsWith('.jsonl'));
    ...
    assert.ok(events.some(event => event.kind === 'entry' && event.module.path === fs.realpathSync.native(child)));
  } finally { fs.rmSync(temp, { recursive: true }); }
});